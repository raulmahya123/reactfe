...
const PressRelease = () => {
  ...
  return (
<section id="press" className="scroll-mt-24 py-16">
      ...
      {Object.keys(pressItems).map((key) => {
        ...
        return (
          ...
          >
            ...
            {/* Category */}
            <span className="inline-block text-xs font-semibold bg-[#C6A75E]/10 text-[#C6A75E] px-3 py-1 rounded-md mb-3">
              {item.category}
            </span>
    ...
    {selectedKey && (
      ...
      >
        <motion.div
          ...
          exit={{ y: 20, opacity: 0 }}
          className="bg-white max-w-2xl w-full rounded-lg p-6 shadow-xl relative"
        >
          <button
            onClick={() => setSelectedKey(null)}
            className="absolute top-3 right-3 text-gray-500 hover:text-[#2F3E34]"
          >
            <X size={20} />
          </button>

          <h3 className="text-xl md:text-2xl font-bold text-[#2F3E34] mb-3">
            {pressItems[selectedKey].title}
          </h3>

          <p className="text-gray-600 whitespace-pre-line leading-loose text-justify">
